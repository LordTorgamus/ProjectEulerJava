 ...
 */
public class Problem0018 extends EulerProblem {
    static int[][] triangle;
    ...
    protected long doProblemSpecificStuff() {
        for(int currentRow = (triangle.length - 1) - 1; currentRow >= 0; currentRow--) {
            int childRow = currentRow + 1;
            for(int node = 0; node < triangle[currentRow].length; node++) {
                int leftChildPosition = node;
                int rightChildPosition = node + 1;
                triangle[currentRow][node] += Math.max(triangle[childRow][leftChildPosition], triangle[childRow][rightChildPosition]);
            }
        }
